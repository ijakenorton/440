	...
	while (size_written < count) {
		...
		if (*f_pos != 0 && begin_offset == 0) {
			if (list_is_last(&curr->list, &asgn1_device.mem_list)) {
				...
				if (!curr->page) {
					kmem_cache_free(asgn1_device.cache,
							curr);
					pr_err("Failed to allocate memory");
					return size_written > 0 ? size_written :
								  -ENOMEM;
...
int __init asgn1_init_module(void)
{
	...
	if (IS_ERR(asgn1_device.class = class_create(MYDEV_NAME))) {
		...
		rv = (int)PTR_ERR(asgn1_device.class);
	...
	if (IS_ERR(asgn1_device.device = device_create(asgn1_device.class, NULL,
		...
		rv = (int)PTR_ERR(asgn1_device.device);
	...
	if (IS_ERR(asgn1_device.cache =
			   kmem_cache_create("cache", sizeof(page_node), 0,
					     SLAB_HWCACHE_ALIGN, NULL))) {
		pr_err("kmem_cache_create failed\n");
		rv = (int)PTR_ERR(asgn1_device.cache);
